"""Stealth Browser CLI -- main entry point.
...
CLI cross-process session persistence:
  - CLISessionManager: file-based persistence (~/.stealth-browser/sessions.json)
...
from stealth_browser.models import BrowserInstance
from stealth_browser.cli.session_manager import SessionContext, UnifiedSessionManager
...
@click.group()
def cli():
    """Stealth-Browser CLI -- atomic command mode"""
...
@cli.command()
@click.option("--path", help="Custom install path (default: ~/.claude/skills/stealth-browser/)")
...
def _create_shim_unix(cli_path: Path, force: bool) -> tuple[Path, bool]:
    """Create ~/.local/bin/stealth-browser shim (macOS/Linux)."""
    bin_dir = Path.home() / ".local" / "bin"
    bin_dir.mkdir(parents=True, exist_ok=True)
    shim_path = bin_dir / "stealth-browser"
...
def _create_shim_windows(cli_path: Path, force: bool) -> tuple[Path, bool]:
    """Create %APPDATA%\\Python\\Scripts\\stealth-browser.bat shim (Windows)."""
    scripts_dir = Path(os.environ.get("APPDATA", str(Path.home()))) / "Python" / "Scripts"
    scripts_dir.mkdir(parents=True, exist_ok=True)
    shim_path = scripts_dir / "stealth-browser.bat"
...
def _install_skill(custom_path=None, force=False):
    ...
    import stealth_browser.skill as skill_mod
    ...
    if custom_path:
        ...
        target_dir = Path.home() / ".claude" / "skills" / "stealth-browser"
    ...
    # Create ~/.local/bin/stealth-browser shim
    shim_path, shim_updated = _create_shim(target_dir / "cli.py", force=force)

    payload = {
        "status": "installed",
        "message": "Stealth Browser skill installed successfully.",
        "path": str(target_dir),
        "shim": str(shim_path),
