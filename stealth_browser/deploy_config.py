...
Config precedence (unchanged from existing system):
  ...
  3. YAML config (~/.stealth-browser/config.yaml) — EXTENDED with new sections
...
CONFIG_DIR = Path.home() / ".stealth-browser"
...
class DeployConfig:
    ...
    k8s_namespace: str = "stealth-browser"
...
def load_deploy_config() -> DeployConfig:
    ...
    the existing ~/.stealth-browser/config.yaml file. Falls back to defaults
...
def generate_skill_config(cfg: "DeployConfig", path: Path | None = None) -> Path:
    ...
    Writes flat YAML (no namespace) to ~/.stealth-browser/skill.yaml by default.
    ...
    Args:
        ...
        path: Override output path. Defaults to ~/.stealth-browser/skill.yaml.
