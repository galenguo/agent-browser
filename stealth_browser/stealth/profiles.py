...
variable ``STEALTH_BROWSER_STEALTH_PROFILE``.  Presets:
...
def profile_from_env() -> StealthProfile:
    """Read ``STEALTH_BROWSER_STEALTH_PROFILE`` env var and return the profile.

    Defaults to ``"minimal"`` when unset, and warns on unrecognised values.
    """
    name = os.getenv("STEALTH_BROWSER_STEALTH_PROFILE", "minimal")
    try:
        return resolve_stealth_profile(name)
    except ValueError:
        logger.warning(
            "Unrecognised STEALTH_BROWSER_STEALTH_PROFILE='%s', falling back to 'full'",
            name,
        )
        return FULL_PROFILE
