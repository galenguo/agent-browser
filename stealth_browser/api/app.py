"""FastAPI server -- HTTP REST API for stealth-browser.
...
from stealth_browser.api.auth import load_keys, require_api_key
from stealth_browser.models import (
    AgentConfig,
    ClickRequest,
    EvaluateRequest,
...
from stealth_browser.session.pool_manager import SessionPoolManager
from stealth_browser.state.store import KEY_ALLOCATIONS

logger = logging.getLogger(__name__)

# Ensure profile storage path is writable (defaults to /data which may not exist)
if not os.environ.get("PROFILE_STORAGE"):
    os.environ["PROFILE_STORAGE"] = os.path.join(tempfile.gettempdir(), "stealth-browser-profiles")
...
# ── Auth dependency ──────────────────────────────────────────────────
# require_api_key is imported from stealth_browser.api.auth (validates against keys.yaml)
...
class KeyManager:
    ...
    def __init__(self, store=None):
        ...
        from stealth_browser.state.store import create_state_store
...
async def _startup():
    ...
    os.environ.setdefault("VNC_BASE_URL", "https://stealth-browser-vnc.vpc-dale.gimcyun.com")
    logger.info("Starting Stealth Browser API server...")
    pool = get_pool()
    pool.start()
    ...
    try:
        ...
        from stealth_browser.state.store import InMemoryStateStore
...
async def browser_proxy(session_id: str, path: str, request: Request,
    ...
    if isinstance(instance, K8sBrowserInstance):
        target_base = instance.novnc_url or f"http://{instance.pod_name}.stealth-browser-browser:6080"
    else:
    ...
    async with httpx.AsyncClient(timeout=httpx.Timeout(120.0)) as client:
        try:
            ...
            return Response(content=resp.content, status_code=resp.status_code,
                            ...
                            headers={"X-Proxy-By": "stealth-browser-api"})
        ...
        for session in pool.sessions.values():
            if session and session.vnc_token == vnc_token:
                ...
                if isinstance(instance, K8sBrowserInstance) and instance.pod_name:
                    ns = os.environ.get("BR_NAMESPACE", "stealth-browser")
                    return f"http://{instance.pod_name}.stealth-browser-br-headless.{ns}.svc.cluster.local:6080"
