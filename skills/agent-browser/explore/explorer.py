...
async def _detect_frameworks(page) -> List[str]:
    """
    检测前端框架（使用 querySelector，不暴露全局变量指纹）。

    隐匿性：使用 DOM 查询而非 window.__VUE__ 等全局变量检测。
    """
    js = """
    (() => {
        const frameworks = [];
        // Vue: 检查 __vue__ 属性（通过 DOM 而非 window）
        if (document.querySelector('[data-v-]') ||
            document.querySelector('[data-server-rendered]')) {
            frameworks.push('vue');
        }
        // React: 检查 __reactFiber 属性
        const root = document.getElementById('root') || document.getElementById('app');
        if (root && (root._reactRootContainer ||
            Object.keys(root).some(k => k.startsWith('__react')))) {
            frameworks.push('react');
        }
        // Angular: 检查 ng-version 属性
        if (document.querySelector('[ng-version]')) {
            frameworks.push('angular');
        }
        // Next.js: 检查 __NEXT_DATA__
        if (document.getElementById('__NEXT_DATA__')) {
            frameworks.push('nextjs');
        }
        // Nuxt: 检查 __NUXT__
        if (document.getElementById('__NUXT_DATA__') ||
...
async def _detect_stores(page) -> List[str]:
    """检测 Pinia/Vuex Store（被动检测，不主动注入）"""
    js = """
    (() => {
        const stores = [];
        // Pinia: 通过 DOM 遍历（不暴露全局变量）
        const app = document.getElementById('app');
        if (app && app.__vue_app__) {
            try {
                const pinia = app.__vue_app__.config.globalProperties.$pinia;
                if (pinia) {
                    stores.push(...Object.keys(pinia.state.value));
                }
            } catch {}
        }
        return stores;
