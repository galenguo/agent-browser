...
Ported from OpenCLI's src/analysis.ts (~171 lines).
...
class InferredCapability:
    ...
    name: str                        # Human-readable name, e.g., "搜索职位"
...
_CAPABILITY_TEMPLATES = {
    'search': '{site}搜索{term}',
    'list': '{site}{resource}列表',
    'detail': '{site}{resource}详情',
    'feed': '{site}{resource}动态',
    'profile': '{site}用户信息',
}
...
def infer_capability_name(
    ...
    if goal:
        name = f'{site}{goal}'
        desc = f'通过{endpoint_url.split("/")[-1]}获取{goal}相关数据'
    elif has_title and has_url:
        name = _CAPABILITY_TEMPLATES['list'].format(site=site, resource='内容')
        desc = f'获取{site}的内容列表，包含标题、链接等字段'
    else:
        name = _CAPABILITY_TEMPLATES['list'].format(site=site, resource='数据')
        desc = f'从{site}提取结构化数据'
...
_SITE_ALIASES = {
    'zhipin.com': ('boss', '直聘', 'zhipin'),
    'boss.zhipin.com': ('boss', '直聘', 'zhipin'),
