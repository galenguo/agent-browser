...
Stealth Browser extends **browser-use** with industrial-grade anti-detection capabilities, a YAML pipeline engine, site exploration, and adapter synthesis. It is designed for **browser-use power users** who hit detection walls when automating interactions with protected websites.
...
Stealth Browser includes a **Claude Code skill** that lets you control browsers directly from conversation:
...
The skill auto-detects your environment, installs missing dependencies, and guides you through the ReAct loop (Observe -> Reason -> Act -> Check). See [skill/SKILL.md](stealth_browser/skill/SKILL.md) for the full skill definition.

#### Skill Configuration (`~/.stealth-browser/skill.yaml`)

The skill reads client config from `~/.stealth-browser/skill.yaml` (flat YAML keys):
...
```bash
# Local mode
python -m stealth_browser.skill.scripts.setup --mode local

# Remote AIO (Docker/K8s all-in-one)
python -m stealth_browser.skill.scripts.setup --mode remote-aio \
  --api-url http://my-server:8000 --vnc-url http://my-server:6080

# Remote distributed
python -m stealth_browser.skill.scripts.setup --mode remote-distributed \
  --api-url http://my-gateway:8000
```

Server admins can also generate a `skill.yaml` from a `DeployConfig` programmatically using `generate_skill_config()` in `stealth_browser.deploy_config`.
...
```bash
# The extension ships inside the pip package at stealth_browser/extension/
# Load it in Chrome: chrome://extensions/ -> Developer mode -> Load unpacked
# Select: <python-site-packages>/stealth_browser/extension/
```

When the extension is connected, Stealth Browser automatically uses **Extension mode** (your real Chrome browser) instead of launching a separate browser instance.
...
├── client.py        # StealthBrowser OOP interface (session tracking, context manager)
...
| Feature | browser-use | Stealth Browser |
|---------|------------|-------------|
