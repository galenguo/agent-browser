...
echo "============================================="
echo "  Stealth Browser on kind + Traefik Deployer"
echo "============================================="
cd "$(dirname "$0")"
...
kind load docker-image stealth-browser-api:latest --name stealth-browser 2>/dev/null || \
  { echo "WARNING: stealth-browser-api image not found locally. Build it first with: cd deploy/docker && bash build-local.sh"; }
kind load docker-image stealth-browser-browser:latest --name stealth-browser 2>/dev/null || \
  { echo "WARNING: stealth-browser-browser image not found locally. Build it first."; }

# ── Step 5: Apply Stealth Browser manifests ────────────────
echo ""
echo "[5/6] Applying Stealth Browser manifests..."
...
kubectl rollout status deployment/stealth-browser-api -n stealth-browser --timeout=120s || true
...
kubectl get nodes
echo ""
echo "Stealth Browser pods:"
kubectl get pods -n stealth-browser -o wide
echo ""
echo "Gateway & Routes:"
kubectl get gateway,httproute -n stealth-browser 2>/dev/null || true
echo ""
echo "--- Access URLs ---"
echo "  API (HTTPRoute):   http://api.stealth-browser.local/"
echo "  Health check:     http://api.stealth-browser.local/health"
echo "  API (NodePort):    http://localhost:30801/"
echo "  noVNC (NodePort):  http://localhost:30680/vnc.html"
echo ""
echo "--- DNS Configuration ---"
echo '  Add to /etc/hosts:'
echo "    127.0.0.1 api.stealth-browser.local browser-*.stealth-browser.local"
echo ""
echo "--- Test Commands ---"
...
echo ''
echo '  # With API Key:'
echo '  curl -H "X-API-Key: YOUR_KEY" -H "Host: api.stealth-browser.local" \\'
echo '    http://localhost/health'
echo ''
echo '  # Create session:'
echo '  curl -X POST -H "X-API-Key: YOUR_KEY" -H "Host: api.stealth-browser.local" \\'
