#!/usr/bin/env python3
"""Skill CLI -- lightweight CLI client for agent-browser skill.
...
Usage (after install-skill creates the shim):
  ...
  agent-browser extract --type text --session default
  agent-browser run "find Python jobs" --session default --max-steps 10
  agent-browser session create --name default
  agent-browser session list
  agent-browser session destroy default
...
SESSION_CACHE = Path.home() / ".agent-browser" / "skill-session.json"

if platform.system() == "Windows":
    _tmp = Path(os.environ.get("TEMP", str(Path.home())))
    DAEMON_SOCK = _tmp / "agent-browser-daemon.sock"
else:
    DAEMON_SOCK = Path.home() / ".agent-browser" / "skill-daemon.sock"

PID_FILE = Path.home() / ".agent-browser" / "skill-daemon.pid"
LOCK_FILE = Path.home() / ".agent-browser" / "skill-daemon.lock"
...
def cmd_open(args: list[str]) -> None:
    ...
    if not args:
        _err("Usage: agent-browser open <url> [--session <name>]")
    url = args[0]
...
def cmd_click(args: list[str]) -> None:
    ...
    if not args:
        _err("Usage: agent-browser click <ref> [--session <name>]")
    ref = args[0]
    session_name = _flag(args, "--session")
    sid = _require_session(session_name)
    body: dict[str, Any] = {}
...
def cmd_fill(args: list[str]) -> None:
    ...
    if len(args) < 2:
        _err("Usage: agent-browser fill <ref> <text> [--session <name>]")
    ref = args[0]
...
def cmd_press(args: list[str]) -> None:
    ...
    if not args:
        _err("Usage: agent-browser press <key> [--session <name>]")
    key = args[0]
    session_name = _flag(args, "--session")
    sid = _require_session(session_name)
...
def cmd_run(args: list[str]) -> None:
    ...
    if not args:
        _err("Usage: agent-browser run \"<task>\" [--session <name>] [--max-steps N]")
    task = args[0]
    session_name = _flag(args, "--session")
...
def cmd_session(args: list[str]) -> None:
    ...
    if not args:
        _err("Usage: agent-browser session <create|list|destroy> [--name <name>]")
    sub = args[0]
    rest = args[1:]
...
def _require_session(name: str | None) -> str:
    ...
    if not sid:
        ...
        _err(
            ...
            f"Run: agent-browser session create --name {label}"
        )
...
def main() -> None:
    ...
    if handler is None:
        _err(f"Unknown command: {cmd}. Run 'agent-browser --help' for usage.")
